...
import {
  LayoutDashboard, Users, Clock, Scale, BookOpen,
  FileWarning, Shield, LogOut, Globe, Menu, X, ChevronRight,
} from 'lucide-react';
...
const NAV = [
  ...
  { id: 'admin-pending',   label: 'Pending Verification', icon: Clock },
  { id: 'admin-users',     label: 'User Directory',       icon: Users },
  { id: 'admin-lawyers',   label: 'Lawyer Directory',     icon: Scale },
  { id: 'admin-reports',   label: 'User Reports',         icon: FileWarning },
  { id: 'admin-articles',  label: 'Manage Articles',      icon: BookOpen },
];
...
const AdminLayout = ({ children }) => {
  ...
  const NavItem = ({ item }) => {
    const active = page === item.id || (item.id === 'admin-dashboard' && page === 'admin');
    return (
      <button
        ...
        style={{
          ...
          color: active ? '#fb7185' : 'var(--text-1)',
      ...
      <div style={{ padding: '10px 16px 14px', marginBottom: '4px' }}>
        <div style={{
          ...
          <div style={{ overflow: 'hidden' }}>
            ...
            <div style={{ fontSize: '0.72rem', color: '#fb7185', fontWeight: 500 }}>
              🛡️ System Admin
            </div>
          </div>
        </div>
